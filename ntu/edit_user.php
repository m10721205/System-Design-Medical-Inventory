  ...
  if(isset($_POST['update'])) {
    ...
    if(empty($errors)){
       ...
       $status   = remove_junk($db->escape($_POST['status']));
            $sql = "UPDATE users SET name ='{$name}',user_level='{$level}',status='{$status}' WHERE id='{$db->escape($id)}'";
     ...
     <div class="panel panel-default">
       <div class="panel-heading">
        <strong>
          ...
          更新 <?php echo remove_junk(ucwords($e_user['name'])); ?> 
        </strong>
       </div>
       <div class="panel-body">
          <form method="post" action="edit_user.php?id=<?php echo (int)$e_user['id'];?>" class="clearfix">
		    <div class="form-group">
                  <label for="username" class="control-label">員工編號</label>
            ...
            <div class="form-group clearfix">
                    <button type="submit" name="update" class="btn btn-info pull-right" onclick="return confirm('是否確定要執行這個動作？');">更新資料</button>
            </div>
        </form>
       </div>
     </div>
  ...
  <div class="col-md-6">
    <div class="panel panel-default">
      <div class="panel-heading">
        <strong>
          <span class="glyphicon glyphicon-th"></span>
          更改 "<?php echo remove_junk(ucwords($e_user['name'])); ?>" 密碼
      </div>
      <div class="panel-body">
        <form action="edit_user.php?id=<?php echo (int)$e_user['id'];?>" method="post" class="clearfix">
          ...
          <div class="form-group clearfix">
                  <button type="submit" name="update-pass" class="btn btn-danger pull-right" onclick="return confirm('是否確定要執行這個動作？');">更新密碼</button>
          </div>
        </form>
      </div>
    </div>
